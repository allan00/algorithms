package test.WNX;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
import java.io.PrintStream;
import java.net.Socket;
import java.net.SocketTimeoutException;

/**
 * Created by qiuping01.wu on 2015/8/24.
 */
public class WNXClient {
    private Socket client;
    public WNXClient() throws IOException {
        this(Constants.SERVER_IP, Constants.PORT);
//        client = new Socket(Constants.SERVER_IP, Constants.PORT);
//        client.setSoTimeout(10000);
//        System.out.println("客户端启动成功");
    }

    public WNXClient(String ip,int port) throws IOException{
        client = new Socket(ip, port);
        client.setSoTimeout(100000);
        System.out.println("客户端启动成功");
    }

    public void startClient() throws IOException, InterruptedException {
        //获取键盘输入
        BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
        //获取Socket的输出流，用来发送数据到服务端
        PrintStream out = new PrintStream(client.getOutputStream());
        //获取Socket的输入流，用来接收从服务端发送过来的数据
        BufferedReader buf =  new BufferedReader(new InputStreamReader(client.getInputStream()));
        boolean flag = true;
        while(flag){
            Thread.sleep(1000);
            System.out.println("输入金额数字(必须是1000的整数倍)购买，输入\"bye\"退出。");
            String str = 1+"";
//            String str = input.readLine();
            if("bye".equals(str)){
                flag = false;
            }else{
                long startTime = System.currentTimeMillis();
                //发送数据到服务端
                out.println(str);
                try{
                    //从服务器端接收数据有个时间限制（系统自设，也可以自己设置），超过了这个时间，便会抛出该异常
                    String echo = buf.readLine();
                    System.out.println(echo);
                }catch(SocketTimeoutException e){
                    System.out.println("Time out, No response");
                }
                long endTime = System.currentTimeMillis();
                System.out.println("响应时间："+(endTime-startTime)+"ms");
            }
        }
        input.close();
        if(client != null){
            //如果构造函数建立起了连接，则关闭套接字，如果没有建立起连接，自然不用关闭
            client.close(); //只关闭socket，其关联的输入输出流也会被关闭
        }
        System.out.println("客户端停止");
    }

    public static void main(String args[]) throws IOException, InterruptedException {
        BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
        System.out.println("请输入服务器ip:");
        String server_ip = input.readLine();
        System.out.println("请输入服务器端口号:");
        int port = Integer.valueOf(input.readLine());
        new WNXClient(server_ip,port).startClient();
    }

}
